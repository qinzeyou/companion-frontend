...
const onConfirmDateTime = () => {
  formData.value.expireTime = `${currentDate.value.join(
    "-"
...
<template>
  <div class="team-add-page">
    <div class="team-cover-box">
      ...
      <img
        v-if="formData.teamCover != ''"
        class="cover"
        :src="formData.teamCover"
        alt=""
      />
      <div class="bg"></div>
    </div>
        ...
        <van-field name="radio" label="单选框">
          <template #input>
            <van-radio-group v-model="formData.status" direction="horizontal">
              <van-radio
                v-for="status in TeamStatusEnum"
                :key="status.value"
                :name="status.value"
                >{{ status.desc }}</van-radio
              >
            </van-radio-group>
          </template>
        </van-field>
...
.team-cover-box {
  ...
  .bg {
    ...
    top: 0;
    left: 0;
    background: url("http://shq2sf8gm.hn-bkt.clouddn.com/system/default-team-cover.png");
