...
const removeActiveTagName = (tagName: string) => {
    ...
    activeTagNameList.value = activeTagNameList.value.filter(item => item.text !== tagName);
}
    ...
    <form action="/">
        ...
        <van-row v-else class="selected-tag-row" :gutter="[0, 20]">
            <van-col span="6" v-for="tag in activeTagNameList" :key="tag.id">
                <van-tag :show="true" closeable size="medium" type="primary" @close="removeActiveTagName(tag.text)">
                    {{ tag.text }}
                </van-tag>
            </van-col>
        </van-row>
